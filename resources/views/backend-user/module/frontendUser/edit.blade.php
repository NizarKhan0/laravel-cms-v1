...
@section('title', 'Edit Frontend User')

@section('content')
    <!-- Breadcrumb Start -->
    <div x-data="{ pageName: `Edit Frontend User` }" class="mb-6">
                    ...
                    <div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-white/[0.03]">
                        <div class="px-5 py-4 sm:px-6 sm:py-5">
                            <h3 class="text-base font-medium text-gray-800 dark:text-white/90">
                                Edit User: {{ $frontendUsers->email }}
                            </h3>
                            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                                Update the information below to modify this frontend user account.
                            </p>
                        </div>

                        <form action="{{ route('frontend-user.update', $frontendUsers->id) }}" method="POST">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="space-y-6 border-t border-gray-100 p-5 sm:p-6 dark:border-gray-800">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="username" value="{{ old('username', $frontendUsers->username) }}"
                                        placeholder="johndoe"
                                        class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 @error('username') border-error-300 @enderror">
                                    @error('username')
                                ...
                                <div>
                                    ...
                                    <input type="email" name="email" value="{{ old('email', $frontendUsers->email) }}"
                                ...
                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                        Password
                                        <span class="text-gray-400 text-xs font-normal">(Leave blank to keep current password)</span>
                                    </label>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="first_name" value="{{ old('first_name', $frontendUsers->first_name) }}"
                                ...
                                <div>
                                    ...
                                    <input type="text" name="last_name" value="{{ old('last_name', $frontendUsers->last_name) }}"
                                ...
                                <div class="rounded-lg bg-gray-50 p-4 dark:bg-gray-800/50">
                                    <p class="text-sm text-gray-600 dark:text-gray-400">
                                        ...
                                        <span id="fullNamePreview">
                                            {{ old('first_name', $frontendUsers->first_name) }}
                                            {{ old('last_name', $frontendUsers->last_name) }}
                                        </span>
                                    </p>
                                </div>
                            </div>
