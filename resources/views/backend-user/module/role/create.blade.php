                ...
                <div>
                    ...
                    <select name="permissions[]" multiple class="js-permission-select w-full">
                        @foreach ($permissions as $permission)
                            <option value="{{ $permission->name }}" @selected(in_array($permission->name, old('permissions', []), true))>
                                {{ $permission->name }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="guard_name" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-900">
                        @foreach ($guards ?? ['admin'] as $guard)
                            <option value="{{ $guard }}" @selected(old('guard_name', 'admin') == $guard)>{{ $guard }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
