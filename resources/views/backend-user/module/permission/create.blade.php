        ...
        <form action="{{ route('permissions.store') }}" method="POST">
            ...
            <div class="space-y-6 p-6">
                <div>
                    ...
                    <input type="text" name="name" value="{{ old('name') }}" placeholder="module.action"
                ...
                <div>
                    ...
                    <select name="guard_name" class="w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm dark:border-gray-700 dark:bg-gray-900">
                        @foreach ($guards ?? ['admin'] as $guard)
                            <option value="{{ $guard }}" {{ (old('guard_name') === $guard) ? 'selected' : '' }}>{{ $guard }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div class="flex justify-end gap-3 border-t border-gray-100 px-6 py-4 dark:border-gray-800">
                ...
                <button type="submit"
                    class="rounded-lg bg-brand-500 px-4 py-2 text-sm text-white hover:bg-brand-600">Save Permission</button>
            </div>
        </form>
    </div>
@endsection
